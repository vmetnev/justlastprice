...
async function recommendationTrend(req, res) {
    ...
    if (recTrendObjectInDB && recTrendObjectInDB.date === new Date().toISOString().split('T')[0]) {
        ...
        try {
            url = `https://69.147.92.11/v6/finance/quoteSummary/${ticker}?modules=recommendationTrend&ssl=true`
            console.log(url)
            ...
            data = json.quoteSummary.result[0].recommendationTrend.trend
